...
namespace WindowsFormsApp1.Worker_UserControl
{
    public partial class newRequest : UserControl
    {
        ...
        public newRequest(string user_id)
        {
            InitializeComponent();
            userID = user_id;
        }
        ...
        {
            newRequestUserControl();
        }

        public void newRequestUserControl()
        {
            ...
            DataTable dt = bll1.getTableItems("_Order", (" WHERE order_status ='pending' ORDER BY urgent DESC")); 
                    ...
                    if (ADT != null)
                    {
                        if (ADT.Rows.Count > 0)
                        {
                            foreach (DataRow ADR in ADT.Rows)
                            {
                                ...
                                if (adr == odr)
                                {
                                    newRequestLayout item = new newRequestLayout(userID);
